pipeline {
  agent any
  environment {
    dockerhub_username = 'cindyclijsters'
  ...
  stages {
    ...
    stage('Push') {
      steps {
        withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'user3', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD']]) {
          sh """
            echo ${WORKSPACE}
